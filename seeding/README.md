...
- **인스타 발견 탭** (HikerAPI로 동작, App Review 불필요):
  - 키워드 검색 또는 username 직접 입력 → 아래 같은 결과 목록에 팔로워수·소개글·최근
    게시물일·공개 이메일/전화(있는 경우)까지 자동으로 채워짐 → "DB에 등록"
  - 키워드 검색은 후보 계정마다 프로필 조회 + 최근 게시물 조회를 자동으로 하기 때문에
    계정당 약 2회 호출이 추가로 들어갑니다 (여전히 건당 매우 저렴합니다)
  - 키워드 검색 API의 정확한 응답 필드는 문서화가 부족해 방어적으로 파싱하고 있습니다.
    결과가 이상하면 캡처해서 알려주시면 파싱 로직을 조정할 수 있습니다.
