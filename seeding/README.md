...
## 5. 인스타그램 - HikerAPI 키 발급 (현재 이 방식 사용 중)

Meta 공식 API(해시태그 검색, Business Discovery)는 App Review 승인이 있어야 남의 계정을
조회할 수 있어서, 서드파티 서비스인 **HikerAPI**로 전환했습니다. 앱 리뷰 없이 바로 됩니다.

1. https://hikerapi.com 접속 → 가입
2. Tokens 메뉴에서 "Create a token" → Access key 복사
3. 앱 접속 후 **설정 탭 → "인스타그램 - HikerAPI Access Key"**에 붙여넣고 저장
4. 요금: 요청당 과금 (문서 기준 약 $0.0006/요청), 가입 시 무료 요청 제공

### (참고, 현재 미사용) Meta 공식 API 경로

이미 비즈니스/크리에이터 계정 전환 및 페이스북 페이지 연결이 되어있다면, Meta 공식 API로도
시도해볼 수 있습니다. 다만 **본인이 소유하지 않은 계정을 조회하려면 Advanced Access(App
Review) 승인이 반드시 필요**합니다 (비즈니스 인증 + 스크린캐스트 제출 + 1~4주 심사, 반려
가능성 있음). 승인받으면 `lib/instagram.ts`의 기존 함수들을 다시 연결해서 쓸 수 있습니다.
...
6. 발급받은 Access Token과 Business Account ID를 앱 접속 후 **설정 탭**의 해당 항목에
   각각 붙여넣고 저장 (App Review 승인 전에는 조회 시 에러가 납니다)
...
- **인스타 발견 탭** (HikerAPI로 동작, App Review 불필요):
  - 계정명 키워드 검색 / **해시태그 검색** / username 직접 입력, 셋 중 무엇으로 찾든 같은
    결과 목록에 팔로워수·소개글·최근 게시물일·공개 이메일/전화(있는 경우)까지 자동으로
    채워짐 → "DB에 등록"
  - **계정명 키워드 검색**은 username/이름에 키워드가 포함된 계정만 찾아서 모수가 적을 수
    있습니다. **해시태그 검색**은 그 해시태그를 쓴 게시물의 작성자를 찾는 방식이라 훨씬 넓은
    모수를 확보할 수 있어요. 최소/최대 팔로워수 필터는 두 검색 모두에 적용됩니다.
  - **해시태그 검색은 응답 구조가 매우 깊게 중첩되어 있어서** (인스타그램 탐색 피드 형식),
    고정된 경로 대신 재귀 탐색으로 게시물을 찾습니다. 다만 "게시물(id)처럼 보이는 오브젝트에
    직접 붙어있는 user만" 추출하도록 좁혀뒀습니다 — 캡션에 태그된 계정이나 추천 계정처럼
    게시물이 아닌 곳에 있는 username은 걸러내지만, 그래도 이따금 무관한 계정이 섞일 수
    있습니다. 결과를 보고 관련 없는 계정은 그냥 등록하지 않으면 됩니다.
  - 소개글(bio) 텍스트로 검색하는 기능은 없습니다 — 인스타그램 자체의 검색엔진이 계정명/
    실명만 인덱싱하고 bio는 검색 대상이 아니라서, 서드파티로도 우회할 수 없는 부분입니다.
  - 검색은 후보 계정마다 프로필 조회 + 최근 게시물 조회를 자동으로 하기 때문에 계정당 약
    2회 호출이 추가로 들어갑니다 (여전히 건당 매우 저렴합니다)
  - 검색 API들의 정확한 응답 필드는 문서화가 부족해 방어적으로 파싱하고 있습니다. 결과가
    이상하면 캡처해서 알려주시면 파싱 로직을 조정할 수 있습니다.
- **네이버 블로그 탭**: 키워드 + 검색 개수(기본 100, 최대 1000) + 최근 N일 이내 포스팅
  필터 → (선택) 이웃수 직접 입력 → "DB에 등록"
  - 검색 개수를 늘리면 네이버 API를 여러 번 나눠 호출합니다 (100건씩 페이지네이션).
    1000건을 넘게는 네이버 API 자체가 지원하지 않습니다.
...
- 인스타그램은 이제 **HikerAPI(서드파티 유료 서비스)**로 조회합니다. Meta 공식 App Review
  없이 바로 작동하지만, 요청당 과금이라 무제한은 아닙니다. 계속 쓰려면 HikerAPI 계정에
  크레딧을 충전해두셔야 합니다.
  - Meta 공식 API(해시태그 검색, Business Discovery)는 여전히 코드에 남아있지만
    (`lib/instagram.ts`) 지금은 라우트에서 사용하지 않습니다. 나중에 App Review 승인을
    받으시면 그쪽으로 다시 연결할 수 있습니다.
  - HikerAPI는 순차 호출 + 200ms 딜레이를 넣어뒀습니다. 한 번에 너무 많은 username을
